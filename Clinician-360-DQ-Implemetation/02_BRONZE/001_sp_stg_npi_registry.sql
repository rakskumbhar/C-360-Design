...
/*=============================================================================
  ...
  Purpose: Stages NPI data from RAW_INGESTION to BRONZE. Near-replica of RAW
           with type casting. No DQ filtering - all records land with DQ_STATUS=NULL.
=============================================================================*/
...
BEGIN
    ...
    CALL P360_DQ.CONFIG.SP_LOG_STEP_START(:v_step_run_id, :P_RUN_ID, 1, 'STG_NPI_REGISTRY', 'BRONZE', 1);
    ...
    FROM P360_DQ.RAW_INGESTION.RAW_NPI_REGISTRY
    ...
    INSERT INTO P360_DQ.BRONZE.STG_NPI_REGISTRY (
        NPI_NUMBER, PROVIDER_FIRST_NAME, PROVIDER_LAST_NAME, PROVIDER_NAME,
        CREDENTIAL, GENDER, STATE, ZIP_CODE, TAXONOMY_CODE, PHONE,
        NPI_DEACTIVATION_FLAG, ENUMERATION_DATE, SOURCE_SYSTEM,
        ADDRESS_LINE_1, CITY, PROVIDER_TIN,
    ...
    SELECT
        ...
        MD5(COALESCE(NPI_NUMBER,'') || '|' || COALESCE(PROVIDER_NAME,'') || '|' || COALESCE(STATE,'')),
...
EXCEPTION
    WHEN OTHER THEN
        ...
        CALL P360_DQ.CONFIG.SP_LOG_ERROR(:P_RUN_ID, :v_step_run_id, 'STG_NPI_REGISTRY', '', '', :v_err_msg, NULL, 'ERROR');
