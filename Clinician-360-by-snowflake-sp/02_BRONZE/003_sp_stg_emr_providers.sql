/*=============================================================================
  ...
  02_BRONZE/003_sp_stg_emr_providers.sql
  
  Source:  SNOWFLAKE_LEARNING_DB.RAW_INGESTION.PROVIDER_MASTER_RAW
  Target:  P360_SP.BRONZE.STG_EMR_PROVIDERS
=============================================================================*/

USE DATABASE P360_SP;
USE SCHEMA BRONZE;

CREATE OR REPLACE PROCEDURE BRONZE.SP_STG_EMR_PROVIDERS(
    P_RUN_ID    VARCHAR,
...
BEGIN
    ...
    CALL P360_SP.CONFIG.SP_LOG_STEP_START(:v_step_run_id, :P_RUN_ID, 3, 'STG_EMR_PROVIDERS', 'BRONZE', 1);

    IF (:P_MODE = 'INCREMENTAL') THEN
        SELECT COALESCE(MAX(_source_loaded_at), '1900-01-01'::TIMESTAMP_NTZ) INTO v_hwm
        FROM P360_SP.BRONZE.STG_EMR_PROVIDERS;
    ELSE
    ...
    SELECT
        UUID_STRING(), :P_RUN_ID, 'STG_EMR_PROVIDERS', 'PROVIDER_MASTER_RAW',
        ARRAY_COMPACT(ARRAY_CONSTRUCT(
            CASE WHEN npi_number IS NULL THEN 'NPI_NUMBER_NULL' END,
            CASE WHEN LENGTH(npi_number) != 10 THEN 'NPI_INVALID_LENGTH' END,
            CASE WHEN LENGTH(state_code) != 2 AND state_code IS NOT NULL THEN 'INVALID_STATE_CODE' END
        )),
        npi_number,
        OBJECT_CONSTRUCT('npi_number', npi_number, 'provider_id', provider_id, 'facility_name', facility_name),
        CURRENT_TIMESTAMP()
    FROM SNOWFLAKE_LEARNING_DB.RAW_INGESTION.PROVIDER_MASTER_RAW
    WHERE _loaded_at > :v_hwm
      AND (
    ...
    CALL P360_SP.CONFIG.SP_CHECK_DQ_THRESHOLD(:P_RUN_ID, :v_step_run_id, 'PROVIDER_MASTER_RAW', :v_total_count, :v_reject_count);
    ...
    INSERT INTO P360_SP.BRONZE.STG_EMR_PROVIDERS (
        emr_provider_id, npi_number, facility_name, address_line_1, address_line_2,
        city, state_code, zip_code, phone_number, is_accepting_patients,
        provider_status, updated_at, _source_loaded_at, _run_id
    )
    SELECT
        provider_id::VARCHAR,
...
EXCEPTION
    WHEN OTHER THEN
        ...
        CALL P360_SP.CONFIG.SP_LOG_ERROR(:P_RUN_ID, :v_step_run_id, 'STG_EMR_PROVIDERS', '', '', :v_err_msg, NULL, 'ERROR');
